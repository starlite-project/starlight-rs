...
use self::{error::RequestError, ratelimiter_map::RatelimiterMap};
use http::{
...
lazy_static! {
	static ref METRIC_KEY: String =
		env::var("METRIC_KEY").unwrap_or_else(|_| "starlight_http_proxy".into());
}
...
async fn main() -> Result<()> {
	...
	let host = IpAddr::from_str(&host_raw).into_diagnostic()?;
	...
	metrics::set_boxed_recorder(Box::new(recorder)).into_diagnostic()?;
...
async fn shutdown_signal() {
	let mut ctrlc = ctrl_c().expect("failed to install CTRLC listener");
	let mut ctrlbreak = ctrl_break().expect("failed to install CTRLBREAK listener");
...
#[cfg(unix)]
async fn shutdown_signal() {
	let mut sigint = signal(SignalKind::interrupt()).expect("failed to install SIGINT listener");
	let mut sigterm = signal(SignalKind::terminate()).expect("failed to install SIGTERM listener");
