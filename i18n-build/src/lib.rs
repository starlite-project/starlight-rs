use std::{
    ...
    fs::{self, File},
    io::{BufReader, Result as IoResult, Write as IoWrite},
    path::Path,
};
...
fn process_json_files() -> IoResult<Vec<String>> {
...
fn process_json_entry_file(languages: Vec<String>) -> std::io::Result<()> {
    ...
    let _ = definition_file.write(b"#![allow(dead_code)]\n");
